...
\begin{document}
	...
	\begin{frame}
		...
		Per gli adroni privi del quark charm si è ottenuto:
		\begin{itemize}
			\item a = (1.057 $\pm$ 0.014) $\cdot \ 10^5$.
			\item b = (6.2107 $\pm$ 0.0071) $GeV^{-1}$. 
			\item $\tilde{\chi}^2 = 0.16$. 
		\end{itemize}
		Per gli adroni contenenti il quark charm invece è stato fissato il parametro b al valore precedentemente ottenuto per poi determinare a = (3.534 $\pm$ 0.021) $\cdot \ 10^5$.
	...
	\begin{frame}
		\frametitle{Stima dei candidati osservabili}
	...
	\begin{frame}
		...
		\end{equation}
		Dove $N_{LHC} = 10^{10}$, $p_D$ è la probabilità che venga ricostruita la traccia di un deutone proveniente dal decadimento del c-deuteron $p_D = (5.4823 \pm 0.0023) \ \cdot \ 10^{-3}$, $(dN/dy)_{c-d}= (7.072 \pm 0.042) \cdot 10^{-4}$. $\Delta y = 1$, $p_{central}$ è la probabilità che si verifichi una collisione centrale $p_{central} = 5\%$. 
